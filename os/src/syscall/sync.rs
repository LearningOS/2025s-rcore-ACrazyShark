...
pub fn sys_mutex_create(blocking: bool) -> isize {
        ...
        .map(|(id, _)| id)
    {
        ...
        process_inner.banker_mutex.set_max_available(id as usize, 1);
        id
    }
}
/// mutex lock syscall
pub fn sys_mutex_lock(mutex_id: usize) -> isize {
    ...
    let mut process_inner = process.inner_exclusive_access();
    process_inner.banker_mutex.bak_need(tid, mutex_id);
    process_inner.banker_mutex.add_allocation(tid, mutex_id, 1);
    process_inner.banker_mutex.bak_available(tid);
...
pub fn sys_semaphore_create(res_count: usize) -> isize {
    ...
    let process = current_process();
    let mut process_inner = process.inner_exclusive_access();
    let id: usize = if let Some(id) = process_inner
        .semaphore_list
        .iter()
        .enumerate()
        .find(|(_, item)| item.is_none())
        .map(|(id, _)| id)
    {
        ...
        process_inner.banker_semaphore.set_max_available(id as usize, res_count);
        id
    };
...
pub fn sys_semaphore_up(sem_id: usize) -> isize {
    ...
    process_inner.banker_semaphore.bak_allocation(tid, sem_id);
    process_inner.banker_semaphore.add_available(sem_id, 1);
...
pub fn sys_enable_deadlock_detect(_enabled: usize) -> isize {
    ...
    // is_enable = 1 Implement check
    // is_enable = 0 Implement check
    if _enabled == 1 {
        // Implement check deadlock
        let process = current_process();
        let mut inner = process.inner_exclusive_access();
        inner.is_enable = true; // 开启死锁检测
        0
    }else{
        -1
    }
